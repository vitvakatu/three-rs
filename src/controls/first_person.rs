...
impl FirstPerson {
    ...
    pub fn update(
        ...
        self.axes
            .vertical
            .map(|a| if let Some(diff) = input.key_axis(a).timed_state {
        ...
        self.axes
            .forward
            .map(|a| if let Some(diff) = input.key_axis(a).timed_state {
                self.position.x += self.move_speed * diff * dtime * self.yaw.sin();
                self.position.z -= self.move_speed * diff * dtime * self.yaw.cos();
                if self.vertical_move {
                    self.position.y -= self.move_speed * diff * dtime * self.pitch.sin();
                }
            });
        self.axes
            .strafing
            .map(|a| if let Some(diff) = input.key_axis(a).timed_state {
                self.position.x += self.move_speed * diff * dtime * self.yaw.cos();
                self.position.z += self.move_speed * diff * dtime * self.yaw.sin();
            });
